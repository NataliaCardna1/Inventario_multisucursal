...
import { crearOrdenCompra } from '../api/compras'
import type { Proveedor, Sucursal, Producto } from '../types'
...
export default function NuevaCompra() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="mb-5 text-xl font-semibold text-text-primary">Nueva orden de compra</h1>

      <form onSubmit={handleSubmit} className="rounded-2xl bg-surface p-6 shadow-sm">
        <div className="mb-4 grid grid-cols-2 gap-4">
          <div>
            <label className="mb-1.5 block text-xs font-semibold tracking-wide text-text-primary uppercase">Proveedor</label>
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold tracking-wide text-text-primary uppercase">Sucursal destino</label>
        ...
        <label className="mb-1.5 block text-xs font-semibold tracking-wide text-text-primary uppercase">Condiciones de pago</label>
        <input value={condicionesPago} onChange={(e) => setCondicionesPago(e.target.value)}
          className="mb-5 w-full rounded-lg border border-border px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />

        <label className="mb-2 block text-xs font-semibold tracking-wide text-text-primary uppercase">Productos</label>
        {items.map((item, index) => (
          <div key={index} className="mb-2 flex items-center gap-2">
            <select value={item.productoId} onChange={(e) => actualizarItem(index, 'productoId', e.target.value)} required
            ...
            <input type="number" min="0.01" step="0.01" placeholder="Cant." value={item.cantidad}
              onChange={(e) => actualizarItem(index, 'cantidad', e.target.value)} required
              className="w-20 rounded-lg border border-border px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
            <input type="number" min="0" step="0.01" placeholder="Precio" value={item.precioUnitario}
              onChange={(e) => actualizarItem(index, 'precioUnitario', e.target.value)} required
              className="w-28 rounded-lg border border-border px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
            <input type="number" min="0" step="0.01" placeholder="Desc." value={item.descuento}
