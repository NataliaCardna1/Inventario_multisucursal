...
export default function Layout() {
  const { email, rol, sucursalId: usuarioSucursalId, sucursalNombre: usuarioSucursalNombre, logout } = useAuth()
  const { sucursales, sucursalActualId, setSucursalActualId } = useSucursal()
  const esAdmin = rol === 'ADMIN_GENERAL'

  useEffect(() => {
    if (!esAdmin && usuarioSucursalId != null) {
      setSucursalActualId(usuarioSucursalId)
    }
  }, [esAdmin, usuarioSucursalId])
        ...
        <header className="flex h-16 items-center justify-between border-b border-border bg-surface px-6">
          {esAdmin ? (
            ...
            <span className="text-sm font-medium text-text-primary">
              {usuarioSucursalNombre ?? 'Sin sucursal asignada'}
            </span>
          )}
